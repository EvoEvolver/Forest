...
export const CollaborationCursor = Extension.create<CollaborationCursorOptions, CollaborationCursorStorage>({
  ...
  addCommands() {
    return {
      updateUser: attributes => () => {
        ...
        // we don't let the plugin set the awareness state, because it is already set somewhere else
        //this.options.provider.awareness.setLocalStateField('username', this.options.user.name)
        //this.options.provider.awareness.setLocalStateField('color', this.options.user.color)
  ...
  addProseMirrorPlugins() {
    return [
      yCursorPlugin(
        (() => {
            // this state is initialize somewhere else, so we don't need to set it here
